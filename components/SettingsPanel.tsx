                                    ...
                                    {botMemories.length === 0 ? (
                                        ...
                                        botMemories.map(mem => (
                                            <div key={mem.id} className="flex justify-between items-start bg-slate-900 p-2 rounded border border-slate-800">
                                                <div>
                                                    <span className={`text-[9px] uppercase font-bold mr-2 px-1 rounded ${mem.type === 'directive' ? 'bg-red-900 text-red-300' :
                    ...
                    {activeTab === 'mcp' && (
                        <div className="space-y-6">
                            ...
                            <div className="p-4 bg-slate-800 rounded border border-slate-700 mb-4">
                                <label className="text-xs text-cyan-400 font-bold uppercase block mb-2">MCP Server Connection</label>
                                <p className="text-[10px] text-slate-400 mb-2">
                                    Connect to the running MCP Server to enable advanced tools (Council, Memory, etc.).
                                    <br />Default: <code>http://localhost:4000</code>
                                </p>
                                <div className="flex gap-2 mb-2">
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <button onClick={() => quickSetEndpoint('http://localhost:4000')} className="text-[10px] bg-slate-700 hover:bg-slate-600 text-white px-2 py-1 rounded">4000</button>
                                ...
                                <div className="flex justify-end">
                                    <button
                                        onClick={async () => {
                                            const url = settings.mcp.dockerEndpoint || "http://localhost:4000";
                                            const healthUrl = `${url.replace(/\/sse$/, '')}/health`;
                                            try {
                                                const res = await fetch(healthUrl);
                                                if (res.ok) alert("✅ Connected to MCP Server!");
                                                else alert(`❌ Error: ${res.status} ${res.statusText}`);
                                            } catch (e: any) {
                                                alert(`❌ Connection Failed: ${e.message}`);
                                            }
                                        }}
                                        className="text-xs bg-emerald-700 hover:bg-emerald-600 text-white px-3 py-1 rounded font-bold uppercase"
                                    >
                                        Test Connection
                                    </button>
                                </div>
