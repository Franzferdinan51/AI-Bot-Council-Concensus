...
interface ChatWindowProps {
  ...
  sessionStartedAt: number | null; // Timestamp when session started
...
const ChatWindow: React.FC<ChatWindowProps> = ({ 
    messages, activeBots, thinkingBotIds, onSendMessage, statusText, currentTopic, sessionStartedAt, sessionMode, onModeChange, sessionStatus, 
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 relative overflow-y-auto">
        ...
        <header className="bg-slate-900 border-b border-amber-900/50 shadow-lg z-20 shrink-0 relative transition-all duration-300 pt-[env(safe-area-inset-top)]">
            <div className="flex items-center justify-between px-2 md:px-4 h-14 md:h-16 w-full max-w-7xl mx-auto">
                ...
                <div className="flex items-center justify-start gap-2 w-24 md:w-40 shrink-0">
                     <button 
                        ...
                        className={`flex items-center gap-1.5 px-2 md:px-3 py-1.5 rounded shadow-sm transition-all hover:scale-105 active:scale-95 group border ${
                            isSessionActive
                                ? 'bg-red-900/30 border-red-700/50 hover:border-red-500 text-red-400'
                                : 'bg-slate-800 border-amber-900/50 hover:border-amber-500 text-amber-500'
                        }`}
                        title={isSessionActive ? "Open Live Chamber View" : "Open Live Watcher (separate tab on another device recommended)"}
                     >
                         <span className="relative flex h-2 w-2">
                            {isSessionActive ? (
                                ...
                                <>
                                    <span className="relative inline-flex rounded-full h-2 w-2 bg-slate-600 group-hover:bg-amber-400"></span>
                                </>
                            )}
                        </span>
                        <span className={`text-[10px] md:text-xs font-bold uppercase tracking-widest hidden sm:inline`}>
                            {isSessionActive ? '🔴 LIVE' : 'Watch'}
                        </span>
                     </button>
            ...
            {/* SESSION STATUS STRIP — shows when session is active */}
            {isSessionActive && (
                <div className={`flex items-center gap-3 px-3 pb-2 text-[10px] font-mono uppercase tracking-widest transition-all duration-300 ${
                    isPaused ? 'bg-yellow-900/20 text-yellow-400 border-t border-yellow-700/30' :
                    sessionStatus === SessionStatus.OPENING ? 'bg-amber-900/20 text-amber-400 border-t border-amber-700/30' :
                    sessionStatus === SessionStatus.VOTING ? 'bg-purple-900/20 text-purple-400 border-t border-purple-700/30' :
                    'bg-cyan-900/20 text-cyan-400 border-t border-cyan-700/30'
                }`}>
                    <span className={`shrink-0 h-1.5 w-1.5 rounded-full ${isPaused ? 'bg-yellow-400' : sessionStatus === SessionStatus.VOTING ? 'bg-purple-400 animate-pulse' : 'bg-cyan-400 animate-pulse'}`}></span>
                    <span className="font-bold">{sessionStatus === SessionStatus.OPENING ? 'OPENING CHAMBER' : isPaused ? 'SESSION PAUSED' : sessionStatus === SessionStatus.VOTING ? 'VOTING IN PROGRESS' : 'IN SESSION'}</span>
                    <span className="text-slate-500 hidden sm:inline">•</span>
                    <span className="hidden sm:inline text-slate-400">{currentTopic ? currentTopic.substring(0, 50) + (currentTopic.length > 50 ? '...' : '') : 'No topic'}</span>
                    <span className="text-slate-600 hidden md:inline">•</span>
                    <span className="hidden md:inline text-slate-400">{activeBots.filter(b => thinkingBotIds.includes(b.id)).map(b => b.name).join(', ') || 'Councilors seated'}</span>
                    <span className="text-slate-600">•</span>
                    <span className="text-amber-500/70 font-black">{formatElapsed(elapsedSeconds)}</span>
                    <span className="text-slate-600">•</span>
                    <span className="text-slate-500">{messages.length} msgs</span>
                    {debateHeat !== 0 && (
                        <>
                            <span className="text-slate-600">•</span>
                            <span className={debateHeat > 0 ? 'text-emerald-400' : 'text-red-400'}>
                                {debateHeat > 0 ? '🔥 Heated' : '❄️ Cooling'}
                            </span>
                        </>
                    )}
                    {isPaused && (
                        <button onClick={onPauseSession} className="ml-auto text-yellow-400 hover:text-yellow-300 underline text-[9px] uppercase">Resume</button>
                    )}
                </div>
            )}
        </header>
        ...
        <div className="bg-slate-900/80 border-b border-slate-800 px-4 py-2 flex items-center justify-between gap-2 overflow-x-auto">
            <div className="flex items-center gap-2">
                ...
                <button onClick={onOpenLiveSession} className={`text-xs px-2 py-1 rounded transition-colors font-bold ${
                    isSessionActive ? 'bg-red-900/50 hover:bg-red-800 text-red-400 animate-pulse' : 'bg-amber-900/50 hover:bg-amber-800 text-amber-400'
                }`}>
