      ...
      {councilors.map((bot) => {
        ...
        return (
          <div 
            key={bot.id} 
            className={`
                relative flex-shrink-0 w-40 md:w-48 p-2 md:p-3 rounded-lg border transition-all duration-300
                ${isActive ? 'border-amber-400 scale-105 shadow-lg shadow-amber-900/20' : 'border-slate-700 opacity-80 hover:opacity-100'}
                bg-slate-800
            `}
          >
            {isActive && (
            ...
            <div className={`h-1 w-full rounded-full mb-2 bg-gradient-to-r ${bot.color}`}></div>
            
            <div className="flex justify-between items-start mb-1">
                <span className={`text-[10px] md:text-xs font-bold uppercase tracking-wider ${badgeColor}`}>
                    {roleName}
                </span>
            ...
            <h3 className="text-white text-sm md:text-base font-serif font-medium truncate" title={bot.name}>{bot.name}</h3>
